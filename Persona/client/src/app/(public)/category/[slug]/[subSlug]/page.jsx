...
export default function SubcategoryPage() {
  ...
  const fetchProducts = async (pageNumber = 1) => {
    try {
      setLoading(true)
      const res = await getProductsByCategory(slug, subSlug, {
        page: pageNumber,
        limit: 20
      })

      setProducts(res.data || [])
      setPageTitle(res.pageTitle || "Subcategory")
      setPage(res.pagination?.page || 1)
      setTotalPages(res.pagination?.pages || 1)
    } catch (err) {
      console.error("Fetch collection error:", err.message)
      setProducts([])
      setPageTitle("Subcategory Not Found")
    } finally {
      setLoading(false)
    }
  }
  ...
  useEffect(() => {
    setPage(1)
    if (slug && subSlug) fetchProducts(1)
  }, [slug, subSlug])
